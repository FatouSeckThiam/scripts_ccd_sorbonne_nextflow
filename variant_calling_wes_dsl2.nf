...
//MantaChannel = Channel.fromList(inputs) Plus besoin de dupliquer le contenu d'un channel pour l'utiliser 2 fois en entrée avec DSL2
...
process RunMutect1{
	...
	*/
	...
	cpus params.cpus
        ...
        output:
        tuple val(norm), path("./${norm}_call_stats.out"), path("./${norm}_filtered.vcf"), path(tumor_bam) 
...
process Mutect1Filter{
	...
	time "96h"
        ...
        input:
 	tuple val(norm), path(call_stats_out), path(filtered_vcf), path(tumor_bam) 
	//tuple val(norm), path(tumor_bam) 

        output:
	tuple val(norm), file("./${norm}_call_stats.pass.out"), file("./${norm}_filtered_PASS.vcf"), path("./${norm}_mutect1.snvs.pass.vcf") 
...
process RunManta {
	...
	tuple val(norm), path(normal_bam), val(tum), path(tumor_bam) 
        ...
        output:
        tuple val(norm), path("./${norm}_manta/results/variants/candidateSmallIndels.vcf.gz"), path("./${norm}_manta/results/variants/candidateSmallIndels.vcf.gz.tbi"), path(normal_bam), val(tum), path(tumor_bam) 
...
process RunStrelka2{
	...
	time "96h"
        ...
        input:
        tuple val(norm), path(candidateSmallIndels), path(candidateSmallIndels_index), path(normal_bam), val(tum), path(tumor_bam) 

        output:
        tuple val(norm), path("./${norm}_strelka2/results/variants/somatic.indels.vcf.gz"), path("./${norm}_strelka2/results/variants/somatic.indels.vcf.gz.tbi"), path ("./${norm}_strelka.indels.pass.vcf") 
...
process MergeMutectStrelka{
	...
	cpus params.cpus
       ...
       // memory { 10.GB * task.attempt}
        ...
        input:
	tuple val(norm), path(pass_somatic), path(filter_pass_index) , path(mutect1_strelka2) 

        output:
        tuple val(norm), path("./${norm}_snv.indels.wes.variants.vcf") 
...
process Vcf2Maf {
	...
	input:
	tuple val(norm), path(merge_mutect1_strelka2) 
	
	output:

	tuple val(norm), path("./${norm}_vcf2maf.maf") 
...
workflow{
	mutect1_variants_ch=RunMutect1(MutectChannel)
	mutect1_filter_ch=Mutect1Filter(mutect1_variants_ch)
	manta_variants=RunManta(MutectChannel)
	strelka2_variants_ch=RunStrelka2(manta_variants)
	mergeChannel=mutect1_filter_ch.concat(strelka2_variants_ch).groupTuple(by: 0).view()
	all_variants_ch=MergeMutectStrelka(mergeChannel)
	variants_annoted_ch=Vcf2Maf(all_variants_ch)
