...
process Star_Mark_duplicat{
	...
	output:
        tuple val(id), path("${id}_rnaseq_Duplicated_marked_noMultipLog.out"), path("${id}_rnaseq_Duplicated_marked_noMultipProcessed.out.bam")
...
process flagstat_sam{
        ...
        input:
 	tuple val(id), path(rnaseq_Duplicated_removed_noMultipLog_out), path(rnaseq_Duplicated_marked_noMultip_bam) 

        output:
        tuple val(id), path(rnaseq_Duplicated_marked_noMultip_bam), path("${id}_rnaseq_report_before_rmdup_UniqueIdenticalNotMulti.txt") 
...
process remove_duplicat {
	...
	time "72h"
        ...
        input: 
	tuple val(id), path(rnaseq_Duplicated_marked_noMultip_bam), path(rnaseq_before_rmdup_UniqueIdenticalNotMulti_txt) 

	output:
        tuple val(id), path("${id}_rnaseq_Duplicated_removed_nomultiple.bam") 
...
process rmdup_Unique {
	...
	time "72h"
        ...
        input:
	tuple val(id), path(rnaseq_Duplicated_removed_nomultiple_bam) 

        output:
	tuple val(id), path(rnaseq_Duplicated_removed_nomultiple_bam), path("${id}_rnaseq_report_after_rmdup_UniqueIdenticalNotMulti.txt") 
...
process run_htseq {
	...
	time "72h"
        ...
        input:
	tuple val(id), path(rnaseq_Duplicated_removed_nomultiple_bam), path(rnaseq_report_after_rmdup_UniqueIdenticalNotMulti_txt)
        
	output:
        tuple val(id), path("${id}_rnaseq_reverse_rmdup_UniqueIdenticalNotMulti.tsv") 
...
workflow{
	bam_rna_ch=Star_Mark_duplicat(BamChannel_rna)
	sort_sam_rna_ch=flagstat_sam(bam_rna_ch)
	mardup_rna_ch=remove_duplicat(sort_sam_rna_ch)
	sng_rna_ch=rmdup_Unique(mardup_rna_ch)
	sng_rna_ch=run_htseq(sng_rna_ch)
}
