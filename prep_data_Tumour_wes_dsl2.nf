...
process Fastq_Tumour{

      	module "fastp/0.22.0"
	cpus params.cpus

        input:
        tuple val(read1_tum_id), path(fastq1_tum), val(read2_tum_id), path(fastq2_tum), val(id) 

	output:
        tuple val(id), path("./${id}_Tumour_R1.fastq.gz"), path("./${id}_Tumour_R2.fastq.gz"), path("./${id}_Tumour.html"), path("./${id}_Tumour.jason") 
...
process Mapping_Tumour {
        ...
        input:
	tuple val(id), path(Tumour_R1), path(Tumour_R2), path(Tumour_html), path(Tumour_jason) 

        output:
        tuple val(id), path("./${id}_Tumour.bam") 
...
process ARgroup_Tumour {
        ...
        input: 
        tuple val(id), path(Tumour_bam) 

        output:
        tuple val(id), path("./${id}_Tumour_rg_sorted.bam"), path("./${id}_Tumour_rg_sorted.bai") 
...
process Markupduplicate_Tumour{
        ...
        input: 
	tuple val(id), path(Tumour_rg_sorted_bam), path(Tumour_rg_sorted_bai) 
        
	output:
        tuple val(id), path("./${id}_Tumour_markdup_rg_sorted.bam"), path("./${id}_Tumour_markdup_metrics.txt"), path("./${id}_Tumour_markdup_rg_sorted.bai") 
...
process Realign_Intervals_Tum{
        ...
        input:
	tuple val(id), path(Tumour_markdup_rg_sorted_bam), path(Tumour_markdup_metrics_txt), path(Tumour_markdup_rg_sorted_bai) 

        output:
        tuple val(id), path ("./${id}_Tumour.realign.intervals"), path(Tumour_markdup_rg_sorted_bam) 
...
process Apply_Realing_Tum{
        ...
        input:
	tuple val(id), path (Tumour_realign_intervals), path(Tumour_markdup_rg_sorted_bam) 
	
        output:
        tuple val(id), path ("./${id}_Tumour_realign_markdup_rg_sorted.bam") 
...
process Basecallibrator_Tumour{
        ...
        input:
        tuple val(id), path(Tumour_realign_markdup_rg_sorted_bam) 

        output:
        tuple val(id), path("${id}_Tumour_bqsr_realign_markdup_rg_sorted.table"), path(Tumour_realign_markdup_rg_sorted_bam) 
...
process Apply_Calibrator_Tumour{
        ...
        input:
	tuple val(id), path(Tumour_bqsr_realign_markdup_rg_sorted_table), path(Tumour_realign_markdup_rg_sorted_bam) 

        output:
        tuple val(id), path ("./${id}_Tumour_bqsr_realign_markdup_rg_sorted.bam"),  path ("./${id}_Tumour_bqsr_realign_markdup_rg_sorted.bai") 
...
process DOC_Tumour{
        ...
        input:
	tuple val(id), path (Tumour_bqsr_realign_markdup_rg_sorted_bam), path(Tumour_bqsr_realign_markdup_rg_sorted_bai) 

        output:
        tuple val(id), path("./${id}_Tumour_depthofcoverage.sample_summary") 
...
workflow{
	fastq_ideation_tumour_ch=Fastq_Tumour(FastqChannel_Tumour)
	bam_tumour_ch=Mapping_Tumour(fastq_ideation_tumour_ch)
	ARgroup_tum_ch=ARgroup_Tumour(bam_tumour_ch)
	Mardup_Tum_ch=Markupduplicate_Tumour(ARgroup_tum_ch)
	Realign_intervals_Tum_ch=Realign_Intervals_Tum(Mardup_Tum_ch)
	Apply_Realign_Tum_ch=Apply_Realing_Tum(Realign_intervals_Tum_ch)
	Basecall_Tum_ch=Basecallibrator_Tumour(Apply_Realign_Tum_ch)
	Apply_Calibrator_Tum_ch=Apply_Calibrator_Tumour(Basecall_Tum_ch)
	DOC_Tum_ch=DOC_Tumour(Apply_Calibrator_Tum_ch)
}
