...
/*inputChannel = Channel.fromFilePairs("${params.inputDir}/*R{1,2}.fastq.gz") //start from fastq
...
process Mixcr{
	...
	memory "40GB"
    	...
    	input:
    	tuple val(id), path(fastq1), val(id2), path(fastq2) from inputChannel
...
process convert_mixcr{
	...
	input : 
	path(clonotypes) from channel_convert

	output:
	path("Convert_*") into convert_mixcr
...
// convert_mixcr.into permets de copier le convert_mixcr et d'obternir autant de channels qu'on aura besoin d'inputs dans différents process.
...
process CalcBasicStats {
	...
	input:
    	path(Convert_id) from ch_CalcBasicStats
    	errorStrategy "ignore"
    	
	output:  
	path("CalcBasicStats_${Convert_id}_*") into basic_stat_channel
...
process CalcDiversityStats{
	...
	module "vdjtools/1.2.1"
    	...
    	input:
    	path(Convert_id) from ch_CalcDiversityStats
    	errorStrategy "ignore"

    	output:  
	path("CalcDiversityStats_${Convert_id}_*") into diversity_resampled_channel
