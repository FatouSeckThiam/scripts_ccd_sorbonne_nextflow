...
process Pyclone_input{
	...
	input:
	tuple val(id), path(pyclone_input) from Inputs_Pyclone
	
	output:	
	tuple val(id), path("./${id}.h5") into ch_h5_files
...
process Pyclone_output{
	...
	input: 
	tuple val(id), path(h5_file) from ch_h5_files
	
	output:
	tuple val(id), path("./${id}.tsv") into tsv_file
