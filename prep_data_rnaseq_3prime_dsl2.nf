...
process Fastq_rna{
	...
	time "96h"
        input:
        tuple val(read1_id), path(fastq1), val(id) 

	output:
        tuple val(id), path("./${id}_rnaseq_R1.fastq.gz"), path("./${id}.html"), path("./${id}.jason") 
...
process Star_Mapping_rna {
	...
	memory "64G"
        ...
        input:
	tuple val(id), path(rnaseq_R1), path(html), path(jason) 
...
process Apply_BSQR_rna{
	...
	tuple val(id), path(rnaseq_bqsr_sorted_markdup_splitn_table), path(rnaseq_sorted_ar_markdup_splitn_bam) 
        ...
        output:
        tuple val(id), path("./${id}_rnaseq_bqsr_sorted_ar_markdup_splitn.bam"), path("./${id}_rnaseq_bqsr_sorted_ar_markdup_splitn.bai") 
