...
params.samplelist_pvac_gx = " "  // => samplelist_pvactoools_gx
params.baseName = " "
params.inputDir_vcf_gx = " " // => /SCVOL02/run_16/neoepitope_vcf/GX_with_value
params.outputDir_v3 = " "  
params.outputDir_v2 = " "
params.outputDir_v1 =" "
params.VEP_plugins=" "
...
def usage(){
	println("\nCe pipeline est conçu pour prédire des néoépitopes à partir des données de WES et RNA-Seq, en tenant compte des informations sur l'expression des gènes.")
        println(   " Arguments réquis :\n")
        println("  --samplelist_pvac_gx: liste des échantillons")
        println("  --inputDir_vcf_gx : [PATH] chemin d'accès pour les fichiers vcf contenant le gene expression (confert create_vep_vcf_neoepitope_GX.nf)")
...
if (params.help) {
        usage()
        exit(1)
} else if (params.samplelist_vcf_gx == " "|| params.inputDir_vcf_gx == " " || params.outputDir_v3 == " " || params.outputDir_v2 == " " || params.outputDir_v1 == " " || params.VEP_plugins == " ")

{
        println("\nUn ou plusieurs arguments réquis non renseignés, merci de suivre les instructions ci-dessus")
}

log.info "-------  PIPELINE PVACTOOLS RNA-Seq (GX)  --------------"
...
log.info "Input dir        : ${params.inputDir_vcf_gx}"
...
samplelist=file("${params.samplelist_pvac_gx}")
inputs=[]
...
samplelist.withReader() {
	...
	hla_al_II= line.split("\t")[2].split("_")[1]
        vcf_file ="${params.inputDir_vcf_gx}/"+vcf
...
//vcf_channel2=Channel.fromList(inputs) plus necessaire de dupliquer les channels input avec DSL2
...
process run_pvactools2{
	...
	mkdir -p !{id}
        ...
        conda run -p /SCVOL01/Tools/pVACtools-2.0.1 pvacseq run !{params.inputDir_vcf_gx}/!{vcf_file} !{id}_Tumour --normal-sample-name !{id}_Germline !{hla_al_I}!{hla_al_II} NetMHC NetMHCpan NetMHCIIpan ./!{id} -e1 8,9,10 -e2 15 --iedb-install-directory /SCVOL01/Projects/Ideation/refs/new_iebd/ --tdna-vaf 0.05 --expn-val 1
...
process run_pvactools3{
	...
	conda run -p /SCVOL01/Tools/pVACtools-3.0.3 pvacseq run !{params.inputDir_vcf_gx}/!{vcf_file} !{id}_Tumour --normal-sample-name !{id}_Germline !{hla_al_I}!{hla_al_II} NetMHC NetMHCpan NetMHCIIpan ./!{id} -e1 8,9,10 -e2 15 --iedb-install-directory /SCVOL01/Projects/Ideation/refs/new_iebd/ --tdna-vaf 0.05 --expn-val 1
