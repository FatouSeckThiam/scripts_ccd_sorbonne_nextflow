...
process Vcf_expression_annotator{
	...
	output:
	tuple val(id), path("${id}_wes_rna_intersection.vep.gx.vcf"), path(sites_readcount_vcf) into vcf_expression_annotator_channel
...
process Vcf_readcount_annotator{
	...
	input:
	tuple val(id), path(wes_rna_intersection_vep_gx_vcf), path(sites_readcount_vcf) from vcf_expression_annotator_channel

	output:
	tuple val(id), path("${id}_wes_rna_intersection.vep.gx.raf.vcf") into vcf_readcount_annotator_channel
