...
params.outputDir_filtre = " "
params.outputDir_pep = " "
...
def usage(){
	println("\nCe pipeline permets d'appliquer des filtres sur les résultats de prédiction de neoépitopes")
        ...
        println("  --outputDir_filtre : [PATH] chemin d'accès pour les fichiers de sortie")
...
if (params.help) {
        usage()
        exit(1)
} else if (params.samplelist == " "|| params.inputDir == " " || params.outputDir_filtre == " " || params.outputDir_pep == " " || params.VEP_plugins == " ")
...
process Peptide_neo{
	...
	shell:
	"""
	mkdir -p !{params.outputDir_pep}/CMHI
	mkdir -p !{params.outputDir_pep}/CMHII
	...
	cp ./!{id}_uniq_CMHI_MT_Epitopes_Seq.txt !{params.outputDir_pep}/CMHI
	cp ./!{id}_uniq_CMHII_MT_Epitopes_Seq.txt !{params.outputDir_pep}/CMHII
